...
\begin{algorithm}
	...
	\quad
\caption{Diffusion algorithm for inpainting. We denote element-wise multiplication by the $\circ$ operator. The $\mathbf{1}_{M=0}$ function represents a matrix with elements $(i,j)$ set to 1 when $M_{i,j}=0$ and 0 otherwise.}
\label{alg:diffusion}
\end{algorithm}
